...
def get_available_models(return_size=False) -> list:
    ...
    available_models = []
    strucure = huggingface_hub.list_repo_tree("zhwang/HPDv2", path_in_repo="benchmark/benchmark_imgs", repo_type="dataset")
    for file in strucure:
        model_ab_name = os.path.basename(file.path)[:-7]
        if return_size:
            available_models.append(model_ab_dict[model_ab_name] + " (Size: " + str(round(file.size / 1000000, 2)) + "MB)")
        else:
...
def evaluate_benchmark(model_id: str, save_path: str, mode : str = "benchmark", batch_size: int = 20, hps_version: str = "v2.1", attacker: str = "PGD") -> None:
    ...
    """
    utils.download_benchmark_prompts(save_path)

    available_models = get_available_models()

    if model_id in available_models:
        utils.download_benchmark_images(model_id, save_path)
    else:
    ...
    imgs_path = os.path.join(save_path, f'datasets/benchmark/benchmark_imgs/{model_id}')
    data_path = os.path.join(save_path, 'datasets/benchmark')
    
    from . import evaluation as eval
    eval.evaluate(mode=mode, data_path=data_path, root_dir=imgs_path, batch_size=batch_size, hps_version=hps_version, attacker=attacker)
