        ...
        with torch.no_grad():
            for i, batch in enumerate(tqdm(dataloader, desc=f'{model_id} {style}')):
                ...
                with torch.cuda.amp.autocast():
                    ...
                    # CLIP -> HPSv2 same network, different parameters trained toward different target
                    # CLIP: image and text encoder
                    # Image encoder: Input image, output feature vector, vector size = 512
                    # Text encoder: Input text, output feature vector, vector size = 512
                    # image_features: (batch_size, feature_size) (20, 512)
                    # text_features: (batch_size, feature_size) (30, 512)
                    # text_features.T: transposed text_features (feature_size, batch_size) (512, 30)
                    # @: doc product: (batch_size, batch_size) (20, 30)
                    # (i, j) element: image i and text j similarity (or human preference score)
                    logits_per_image = image_features @ text_features.T * 100
                    # Same as
                    # logits_per_image = torch.sum(image_features * text_features, dim=1) # 20
...
def evaluate(mode: str, root_dir: str, data_path: str = os.path.join(root_path,'datasets/benchmark'), checkpoint_path: str = None, batch_size: int = 20, hps_version: str = "v2.1", attacker: str = "PGD") -> None:
    
    # check if the default checkpoint exists
    if not os.path.exists(data_path):
        os.makedirs(data_path)
    
    if hps_version.startswith("v2"):
    ...
    elif hps_version.startswith("v1"):
        import clip
        model, preprocess_val = clip.load("ViT-L/14", device=device)
        # Downloaded from https://github.com/tgxs002/align_sd?tab=readme-ov-file
        checkpoint = torch.load("hpc.pt", weights_only=False, map_location=device)
        model.load_state_dict(checkpoint['state_dict'])

    tokenizer = get_tokenizer(model_name)
    model = model.to(device)
    model.eval() # Disable dropout layers, etc. during evaluation, model.train() to set it back to training mode
    ...
    if mode == 'ImageReward':
        ...
        from hpsv2.attack import attack_rank
        attack_rank(data_path, root_dir, model, batch_size, preprocess_val, tokenizer, device, attacker)
    ...
    elif mode == 'attack_benchmark':
        from hpsv2.attack import attack_benchmark
        attack_benchmark(data_path, root_dir, model, batch_size, preprocess_val, tokenizer, device, attacker)
    elif mode == 'drawbench':
