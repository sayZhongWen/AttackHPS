...
def create_link(source, link_name):
    if os.name == 'nt':
        # On Windows, copy the file or handle accordingly
        shutil.copy(source, link_name)
    else:
        # On Unix-based systems, create a symlink
...
def download_benchmark_prompts(save_path) -> None:
    
    folder_name = os.path.join(save_path, 'datasets/benchmark')
    ...
    for file in hps_prompt_files:
        ...
        if not os.path.exists(os.path.join(folder_name, file)):
            create_link(file_name, os.path.join(folder_name, file))
...
def download_benchmark_images(model_id: str, save_path: str) -> None:
    try:
        ...
        if model_id not in model_ab_dict.keys():
            return
    folder_name = os.path.join(save_path, 'datasets/benchmark/benchmark_imgs/'+ model_id)
    ...
    if not os.path.exists(folder_name):
        with tarfile.open(file_name, 'r:*') as tar:
            tar.extractall(path=os.path.join(save_path, 'datasets/benchmark/benchmark_imgs/'))
